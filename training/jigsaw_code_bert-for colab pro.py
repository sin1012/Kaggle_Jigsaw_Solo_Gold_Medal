# -*- coding: utf-8 -*-
"""jigsaw_code_bert.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ohBQc5OEpsdINOhjQH_1S73-Qv0TzQVE
"""
...
class SynthesicOpenSubtitlesTransform(NLPTransform):
    def __init__(self, always_apply=False, p=0.5):
        ...
        df = pd.read_csv(f'/content/drive/My Drive/jigsaw2020-kaggle-public-baseline/input/open-subtitles-toxic-pseudo-labeling/open-subtitles-synthesic.csv', index_col='id')[['comment_text', 'toxic', 'lang']]
...
def get_train_transforms():
    return albumentations.Compose([
        ExcludeUsersMentionedTransform(p=.95),
        ExcludeUrlsTransform(p=.95),
        ExcludeNumbersTransform(p=.95),
        ExcludeHashtagsTransform(p=.95),
        ExcludeDuplicateSentencesTransform(p=.95),
    ], p=1.0)
...
# %%time
# 
# df_train = pd.read_csv('/content/drive/My Drive/jigsaw2020-kaggle-public-baseline/input/jigsaw-public-baseline-train-data/train_data.csv')
# 
# 
...
df_val = pd.read_csv('/content/drive/My Drive/jigsaw2020-kaggle-public-baseline/input/jigsaw-multilingual-toxic-comment-classification/validation.csv', index_col='id')
...
df_test = pd.read_csv('/content/drive/My Drive/jigsaw2020-kaggle-public-baseline/input/jigsaw-multilingual-toxic-comment-classification/test.csv', index_col='id')
...
class TPUFitter:
    def __init__(self, model, device, config):
        if not os.path.exists('/content/drive/My Drive/Kaggle Comp/MToxic/node_submissions'):
            os.makedirs('/content/drive/My Drive/Kaggle Comp/MToxic/node_submissions')
    ...
    def run_inference(self, test_loader):
        ...
        node_count = len(glob('/content/drive/My Drive/Kaggle Comp/MToxic/node_submissions/*.csv'))
        result.to_csv(f'/content/drive/My Drive/Kaggle Comp/MToxic/node_submissions/submission_{node_count}_{datetime.utcnow().microsecond}_{random.random()}.csv', index=False)
...
class ToxicSimpleNNModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.backbone = transformers.BertModel.from_pretrained(BACKBONE_PATH)
...
submission = pd.concat([pd.read_csv(path) for path in glob('/content/drive/My Drive/Kaggle Comp/MToxic/node_submissions/*.csv')]).groupby('id').mean()
submission['toxic'].hist(bins=100)

submission.to_csv(f'{ROOT_PATH}/bert.csv')
